...
- **Três caminhos de ingestão**, despachados por `meta.tipo` em `ingest/source.py` e em `parse_norma`:
  ...
  2. **Jurisprudência** (`jurisprudencia.py`): súmulas vinculantes servidas do JSON versionado, norma de dispositivo único (`path=enunciado`). Existe porque o parser só produz dispositivo a partir de `Art. N` e um enunciado sairia com **zero dispositivos = sucesso silencioso** (entra no `state.sqlite`, nenhum ponto no índice, nenhum erro).
  3. **Parlamento** (`parlamento_fetcher.py`): regimentos internos do sítio de cada Casa, UTF-8, com `recorte` porque a página serve mais de um documento.
